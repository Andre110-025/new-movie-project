...
const moreTrendingInUs = ref([])
const allMoviesFetched = ref(false) // Flag when no more pages

const getTrendingInUs = async () => {
  if (isFetching.value || allMoviesFetched.value) return

  isFetching.value = true
  try {
    const response = await apiFunction.get(`/movie/top_rated?region=US?page=${page.value}`)
    console.log('more US trending movies(page ' + page.value + '):', response)
    ...
    if (results.length) {
      moreTrendingInUs.value.push(...results.slice(0, moviesPerPage))
      page.value++
    } else {
...
<template>
  <div class="h-[200px] flex justify-center items-center py-6 px-4 bg-[#911b1b]">
    <h2 class="text-[28px] sm:text-[40px] text-white text-center sm:text-left">
      Check out more Popular US Movies here
    </h2>
  </div>
  ...
  <div class="relative w-full max-w-[1250px] mx-auto mt-6 px-3 sm:px-4">
    ...
    >
      <div
        v-for="(img, index) in moreTrendingInUs"
        :key="index"
