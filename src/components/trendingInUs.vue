...
const trendingInUs = ref([])

const getTrendingUs = async (count = 5) => {
  try {
    loading.value = true
    trendingInUs.value = []
    ...
    const response = await apiFunction.get(`/movie/top_rated?region=US`)
    console.log('Us trending movies:', response)

    if (response.status !== 200) {
      throw new Error('Failed to get trending movies')
    }

    trendingInUs.value = response.data.results.slice(0, count)
  } catch (err) {
    console.error(err)
...
<template>
  <div>
    <div class="mt-16 mx-auto max-w-[1250px] flex justify-between items-center px-4">
      <h2 v-if="trendingInUs.length" class="text-2xl font-semibold">Top rated in Us</h2>
      <RouterLink
        to="/moreTrendingInUs"
        class="flex flex-row items-center gap-1"
        v-if="trendingInUs.length"
      >
    ...
    <div class="relative w-full max-w-[1250px] mx-auto mt-6 px-3 sm:px-4">
      <!-- Mobile View for popular us movies -->
      <div
        v-if="trendingInUs.length"
